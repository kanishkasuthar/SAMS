...
import { X, Filter, Search, RotateCcw, Check } from 'lucide-react';
...
const AdvancedFilterDrawer = ({ isOpen, onClose }) => {
  ...
  const handleReset = () => {
    setFilters({ status: [], department: [], manager: [], risk: [], budget: null });
  };

  const statusOptions = ['On Track', 'At Risk', 'Needs Attention', 'Completed'];
  const deptOptions = ['Engineering', 'Marketing', 'Sales', 'Finance', 'HR'];
      ...
      <div style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '32px' }}>
          ...
          <div style={{ marginBottom: 32 }}>
            <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--color-text-main)', marginBottom: 12 }}>Project Status</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {statusOptions.map(opt => (
                ...
                >
                  {filters.status.includes(opt) && <Check size={14} />} {opt}
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Risk Level */}
          <div style={{ marginBottom: 32 }}>
            <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--color-text-main)', marginBottom: 12 }}>Risk Level</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {riskOptions.map(opt => (
                <button 
                  key={opt}
                  ...
                  style={{ 
                    padding: '8px 16px', borderRadius: 20, fontSize: '13px', fontWeight: 600, cursor: 'pointer',
                    backgroundColor: filters.risk.includes(opt) ? 'var(--color-primary)' : 'var(--color-surface-alt)',
                    color: filters.risk.includes(opt) ? 'white' : 'var(--color-text-secondary)',
                    border: filters.risk.includes(opt) ? '1px solid var(--color-primary)' : '1px solid var(--color-border)'
                  }}
                >
                  {opt}
                </button>
